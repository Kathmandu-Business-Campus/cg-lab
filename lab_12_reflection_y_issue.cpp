...
int main() {
    ...
    setcolor(YELLOW);
    for (i = 0; i < 3; i++) {
        reflected[i][0] = 640 - triangle[i][0];
        reflected[i][1] = triangle[i][1];

        int x1 = reflected[i][0] + 320 - 640;
        int y1 = 240 - reflected[i][1];
        int x2 = reflected[(i + 1) % 3][0] + 320 - 640;
        int y2 = 240 - reflected[(i + 1) % 3][1];
        line(x1, y1, x2, y2);
    }

    outtextxy(20, 20, "Reflection along Y-axis");
